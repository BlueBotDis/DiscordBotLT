...
**═════════════════════**
                        **اوامر عادية**
!linkdm  || ارسال رابط السيرفر بالخاص
!link || لينك اي بوت يجب عليك ان تمنشن البوت
!cat || صور قطط كيوت
!sug || للأقتراح
!invites || لمعرفة عدد دعواتك
!invite || لمعرفة عدد دعواتك
!members || معرفة معلومات الاعبين
!rps || حجرة ورقة مقص __قد تكون معطله__
!mar ||  لعبة الزواج
!new || لفتح تذكرة
!counting || للعد
**═════════════════════**
                        **اوامر ادارية**
!giveaway || قيف اواي
!clean || لمسح 12 رسالة فقط
!server || لمعرفة معلومات السيرفر
!gRole || لمعرفة معلومات الرتبة
!id || ملفك
!cc || انشاء الوان 
!closec || قفل الشات
!openc || فتح الشات
!clear 50 || لمسح 50 رسالة
!bc || برودكاست
...
if(!message.member.hasPermission('ADMINISTRATOR')) return message.reply(' **__ليس لديك صلاحيات__**');
 ...
 client.on('message', message => {
  ...
  client.channels.get("501843143625408512").send({embed : banembed})
}
});
...
client.on("ready", () => {
    setInterval(function(){
        client.guilds.get("498078431972556800").roles.find("name", "LegendsRainBow").edit({
            color : "RANDOM"
        });
    }, 1000)
});
...
client.on('voiceStateUpdate', (oldM, newM) => {
  ...
  let ch = oldM.guild.channels.find('name', 'log-us')
  if(!ch) return;
